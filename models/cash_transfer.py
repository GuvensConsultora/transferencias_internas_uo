    ...
    def action_validate(self):
        for rec in self:
            ...
            if transfer_currency != company_currency:
                # Convertimos a moneda de compañía para debit/credit
                company_amount = transfer_currency._convert(
                ...
                line_in = {
                    ...
                    #'amount_currency': rec.amount,      # + en débito
                ...
                line_out = {
                    ...
                    'credit': rec.amount,
                    'currency_id': transfer_currency.id,
                    #'amount_currency': -rec.amount,     # − en crédito
                    'company_id': rec.company_id.id,
                }
            else:
                # Misma moneda que la compañía: amount_currency debe ser 0.0
                line_in = {
                    'name': _('Entrada a %s') % rec.journal_id_to.name,
                    'account_id': acc_to.id,
                    'debit': rec.amount,
                    ...
                    #'amount_currency': 0.0,             # ← clave para que no te anule debit/credit ya que odoo lo realiza solitoooo
                    'company_id': rec.company_id.id,
                }
                line_out = {
                    'name': _('Salida de %s') % rec.journal_id_from.name,
